...
function nip(options) {
  ...
  function processNextFile() {
    ...
    if (options.inPlace && filename != "-") {
      var fs = require('fs');
      options.outStream = fs.createWriteStream(filename + ".tempnip");
    }
    ...
    processer({
      ...
      if (options.inPlace && filename != "-") {
        ...
        if (options.inPlace !== true && typeof options.inPlace === "string" && options.inPlace.trim().length > 0) {
          var suffix = options.inPlace.trim();
          fs.renameSync(filename, filename+"."+suffix);
          fs.renameSync(filename + ".tempnip", filename);
        }
        else {
          fs.renameSync(filename + ".tempnip", filename);
        }
